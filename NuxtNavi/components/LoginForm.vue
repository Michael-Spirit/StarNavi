<template>
  <form novalidate>
    ...
    <div v-show="login_form">
      <div class="inputGroup inputGroup1">
        <label for="loginEmail" id="loginEmailLabel">Email</label>
        <input v-model="login.email" type="email" id="loginEmail" maxlength="254"/>
        <p class="helper helper1">email@domain.com</p>
      </div>
      <div class="inputGroup inputGroup2">
        <label for="loginPassword" id="loginPasswordLabel">Password</label>
        <input v-model="login.password" type="password" id="loginPassword"/>
        <label id="showPasswordToggle" for="showPasswordCheck">Show
          <input id="showPasswordCheck" type="checkbox"/>
          <div class="indicator"></div>
          <a v-on:click="login_form = !login_form" href="javascript:void(0)">Wanna register?</a>
        </label>
      </div>
    </div>
    <div v-show="!login_form">
      <div class="inputGroup inputGroup1">
        ...
        <input v-model="register.email" type="email" id="registerEmail" maxlength="254"/>
        <p class="helper helper1">email@domain.com</p>
      </div>
      <div class="inputGroup inputGroup2">
        <label for="registerPassword1" id="registerPassword1Label">Password</label>
        <input v-model="register.password1" type="password" id="registerPassword1"/>
      </div>
      <div class="inputGroup inputGroup2">
        <label for="registerPassword2" id="registerPassword2Label">Repeat Password</label>
        <input v-model="register.password2" type="password" id="registerPassword2"/>
        <a v-on:click="login_form = !login_form" href="javascript:void(0)">Back to login</a>
      </div>
    </div>

    <div class="inputGroup inputGroup3">
      <button v-if="login_form" v-on:click="vLogin()" type="button" id="login">Log in</button>
      <button v-if="!login_form" v-on:click="vRegister()" type="button" id="register">Register</button>
    </div>
  </form>
</template>
    ...
    export default {
      ...
      mounted() {
          var emailLabel = document.querySelector('#loginEmailLabel'),
            register_emailLabel = document.querySelector('#registerEmailLabel'),
            email = document.querySelector('#loginEmail'),
            register_email = document.querySelector('#registerEmail'),
            passwordLabel = document.querySelector('#loginPasswordLabel'),
            register_passwordLabel = document.querySelector('#registerPasswordLabel'),
            password = document.querySelector('#loginPassword'),
            register_password = document.querySelector('#registerPassword1'),
            register_password2 = document.querySelector('#registerPassword2'),
          ...
          var eyeLCoords, eyeRCoords, noseCoords, mouthCoords, eyeLAngle, eyeLX, eyeLY, eyeRAngle, eyeRX, eyeRY,
            noseAngle, noseX, noseY, mouthAngle, mouthX, mouthY, mouthR, chinX, chinY, chinS, faceX, faceY, faceSkew,
          ...
          function initLoginForm() {
            ...
            // handle events for email input
              ...
              register_email.addEventListener('focus', onEmailFocus);
              register_email.addEventListener('blur', onEmailBlur);
              register_email.addEventListener('input', onEmailInput);

              emailLabel.addEventListener('click', onEmailLabelClick);
              register_emailLabel.addEventListener('click', onEmailLabelClick);

              password.addEventListener('focus', onPasswordFocus);
              password.addEventListener('blur', onPasswordBlur);
              register_password.addEventListener('focus', onPasswordFocus);
              register_password.addEventListener('blur', onPasswordBlur);
              register_password2.addEventListener('focus', onPasswordFocus);
              register_password2.addEventListener('blur', onPasswordBlur);
      ...
      methods: {
        vLogin() {
          axios.post("http://localhost:8000/rest-auth/login/", this.login, { crossdomain: true })
                .then(response => {
                  this.$store.commit('localStorage/setAuth', response.data.token);
                  this.$store.commit('localStorage/setUser', response.data.user)
                })
                .catch(error => {
                  // alert(error.response.data);
                  alert(error.response.data[Object.keys(error.response.data)[0]][0]);
                });
        },
        vRegister() {
                axios.post("http://localhost:8000/rest-auth/registration/", this.register, { crossdomain: true })
                .then(response => {
